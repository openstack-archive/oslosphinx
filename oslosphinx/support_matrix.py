...
STATUS_DICT = {
    ...
    SupportMatrixImplementation.STATUS_MISSING: u"\u2716",
...
class SupportMatrixDirective(rst.Directive):
    ...
    def _get_targets(self, cfg):
        ...
        targets = {}
        driver_target = "driver-impl"

        for item in cfg.options("targets"):
            if not item.startswith(driver_target):
                continue
    ...
    def _get_features(self, cfg, targets):
        ...
        for section in cfg.sections():
            ...
            for item in cfg.options(section):
                ...
                notes_key = driver_notes + item[len(driver_notes):]
                notes = None
